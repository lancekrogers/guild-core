...
func (p *GoParser) Parse(ctx context.Context, filename string, content []byte) (*ParseResult, error) {
...
func (p *GoParser) GetFunctions(result *ParseResult) ([]*Function, error) {
...
func (p *GoParser) GetClasses(result *ParseResult) ([]*Class, error) {
	file, ok := result.AST.(*ast.File)
	if !ok {
		return nil, gerror.New(gerror.ErrCodeInternal, "invalid AST type for Go parser", nil).
			WithComponent("go_parser").
...
func (p *GoParser) GetImports(result *ParseResult) ([]*Import, error) {
	file, ok := result.AST.(*ast.File)
	if !ok {
		return nil, gerror.New(gerror.ErrCodeInternal, "invalid AST type for Go parser", nil).
			WithComponent("go_parser").
...
func (p *GoParser) FindSymbol(result *ParseResult, symbolName string) ([]*Symbol, error) {
	file, ok := result.AST.(*ast.File)
	if !ok {
		return nil, gerror.New(gerror.ErrCodeInternal, "invalid AST type for Go parser", nil).
			WithComponent("go_parser").
...
// extractFunction converts an ast.FuncDecl to a Function
func (p *GoParser) extractFunction(funcDecl *ast.FuncDecl, fset *token.FileSet, funcDocs map[string]string) *Function {
...
// extractStruct converts struct information to a Class
func (p *GoParser) extractStruct(typeSpec *ast.TypeSpec, structType *ast.StructType, fset *token.FileSet, packageName string) *Class {
...
func (p *GoParser) buildFunctionSignature(fn *Function) string {
