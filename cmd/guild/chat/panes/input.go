...
type InputPane interface {
	...
	// Completion
	ShowCompletions(results []common.CompletionResult)
	HideCompletions()
...
type inputPaneImpl struct {
	*layout.BasePane
	...
	completions        []common.CompletionResult
	completionIndex    int
...
func NewInputPane(width, height int, completionEnabled bool) (InputPane, error) {
	...
	pane := &inputPaneImpl{
		BasePane:         basePane,
		textarea:         ta,
		completions:      make([]common.CompletionResult, 0),
...
func (ip *inputPaneImpl) ShowCompletions(results []common.CompletionResult) {
...
func (ip *inputPaneImpl) HideCompletions() {
	...
	ip.completions = make([]common.CompletionResult, 0)
	ip.completionIndex = 0
}
