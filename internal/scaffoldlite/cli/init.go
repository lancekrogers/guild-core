...
func ExecuteInit(ctx context.Context, options *InitOptions) error {
    if options.ProjectName == "" { return gerror.New(gerror.ErrCodeValidation, "project name cannot be empty", nil) }
    if options.OutputDirectory == "" { return gerror.New(gerror.ErrCodeValidation, "output directory cannot be empty", nil) }

    templateFS, err := templ.GetEmbeddedTemplatesFS()
    if err != nil { return gerror.Wrap(err, gerror.ErrCodeInternal, "failed to access templates") }
    ...
    vars := map[string]any{}
    for k, v := range options.Variables { vars[k] = v }
    // Add common defaults
    if _, ok := vars["campaign_name"]; !ok { vars["campaign_name"] = options.ProjectName }
    if _, ok := vars["project_name"]; !ok { vars["project_name"] = options.ProjectName }
    ...
    if stats.FilesSkipped > 0 { fmt.Printf("   Files skipped: %d\n", stats.FilesSkipped) }
    if stats.FilesFailed > 0 { fmt.Printf("   Files failed: %d\n", stats.FilesFailed) }
    return nil
}
