...
func (sv *semanticValidator) SetTemplateFS(fsys fs.FS) {
    if tc, ok := sv.templateChecker.(*templateChecker); ok { tc.templateFS = fsys }
}
...
type TemplateChecker interface{ CheckTemplates(ctx context.Context, recipe *Recipe) []ValidationError }
...
func (tc *templateChecker) CheckTemplates(ctx context.Context, recipe *Recipe) []ValidationError {
    var errors []ValidationError
    if tc.templateFS == nil { return errors }
    templatesSeen := make(map[string]bool)
    for i, file := range recipe.Files {
        select { case <-ctx.Done():
            errors = append(errors, ValidationError{Field: "validation", Message: "validation cancelled", Code: "VALIDATION_CANCELLED"}); return errors
        default: }
...
func (tc *templateChecker) validateTemplateSyntax(templatePath, templateName string) []ValidationError {
    ...
    if err != nil { errors = append(errors, ValidationError{Field: "template", Message: fmt.Sprintf("failed to read template: %v", err), Value: templateName, Code: ErrCodeFileRead}); return errors }
...
func getValidationTemplateFuncMap() template.FuncMap {
    return template.FuncMap{
        "default": func(def, val any) any { if val == nil { return def }; return val },
    }
}

// Path and variable validators (minimal implementations)
type PathValidator interface{ CheckPaths(recipe *Recipe) []ValidationError }
type VariableChecker interface{ CheckVariables(recipe *Recipe) []ValidationError }
