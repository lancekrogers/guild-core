...
func (p *yamlParser) parseWithTimeout(ctx context.Context, data []byte, filename string) (*Recipe, error) {
    ...
    defer cancel()
    type parseResult struct { recipe *Recipe; err error }
...
func (p *yamlParser) parseYAML(data []byte, filename string) (*Recipe, error) {
    if isTreeFormat(data) {
        ...
        if err != nil { return nil, gerror.Wrap(err, ErrCodeYAMLParse, "failed to parse tree format").WithDetails("file", filename) }
        return recipe, nil
    }
    var recipe Recipe
    ...
    if err := p.validateRequiredFields(&recipe); err != nil { return nil, err }
...
type cacheEntry struct { recipe *Recipe; timestamp time.Time }

func (pc *ParserCache) Get(key string) (*Recipe, bool) {
    pc.mu.RLock(); defer pc.mu.RUnlock()
    entry, ok := pc.cache[key]
    if !ok { return nil, false }
    if time.Since(entry.timestamp) > pc.ttl { return nil, false }
    return entry.recipe, true
}
func (pc *ParserCache) Set(key string, r *Recipe) { pc.mu.Lock(); pc.cache[key] = &cacheEntry{recipe: r, timestamp: time.Now()}; pc.mu.Unlock() }
...
func (p *yamlParser) applyDefaults(recipe *Recipe) {
    ...
    if recipe.TemplatesDir == "" { recipe.TemplatesDir = "" }
}
