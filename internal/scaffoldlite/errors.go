...
func (ve ValidationErrors) Error() string {
    if len(ve) == 0 { return "validation failed with no specific errors" }
    if len(ve) == 1 { return ve[0].Error() }
...
func ErrFileExists(path string) error { return gerror.New(ErrCodeFileExists, "file already exists and overwrite is disabled", nil).WithDetails("path", path) }
func ErrTemplateRender(template string, err error) error { return gerror.Wrap(err, ErrCodeTemplateRender, "template rendering failed").WithDetails("template", template) }
func ErrFileWrite(path string, err error) error { return gerror.Wrap(err, ErrCodeFileWrite, "failed to write file").WithDetails("path", path) }
func ErrFileRead(path string, err error) error { return gerror.Wrap(err, ErrCodeFileRead, "failed to read file").WithDetails("path", path) }
func ErrYAMLParse(path string, err error) error { return gerror.Wrap(err, ErrCodeYAMLParse, "failed to parse YAML").WithDetails("path", path) }
...
func ErrPermission(path string, err error) error { return gerror.Wrap(err, ErrCodePermission, "permission denied").WithDetails("path", path) }
func ErrInvalidPath(path string, reason string) error { return gerror.New(ErrCodeInvalidPath, "invalid file path", nil).WithDetails("path", path).WithDetails("reason", reason) }
