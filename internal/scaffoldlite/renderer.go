...
func (tc *TemplateCache) Clear() { tc.mu.Lock(); tc.cache = make(map[string]*templateCacheEntry); tc.mu.Unlock() }
...
func (tc *TemplateCache) evictLRU() {
    ...
    if oldestName != "" { delete(tc.cache, oldestName) }
}
...
func convertToYAML(v any) string {
    ...
    case []any:
        var items []string
        for _, item := range val { items = append(items, "- "+convertToYAML(item)) }
        return strings.Join(items, "\n")
    case map[string]any:
        var items []string
        for key, value := range val { items = append(items, fmt.Sprintf("%s: %s", key, convertToYAML(value))) }
        return strings.Join(items, "\n")
    default:
        return fmt.Sprintf("%v", val)
    }
