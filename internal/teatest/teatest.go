...
// WithInitialTermSize sets an initial terminal size for a test model.
...
// WithFinalTimeout allows to set a timeout for how long FinalModel and FinalOutput should wait for the program to complete.
...
// This method only returns once the program has finished running or when it times out.
...
// RequireEqualOutput is a helper function to assert the given output is the expected from the golden files,
// printing its diff in case it is not.
//
