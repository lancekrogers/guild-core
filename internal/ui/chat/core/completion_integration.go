...
func (ci *CompletionIntegration) Complete(ctx context.Context, input string, cursorPos int) ([]common.CompletionResult, error) {
	if ci.completionEngine == nil {
...
func (ci *CompletionIntegration) getSuggestionsWithContext(ctx context.Context, input string) ([]common.CompletionResult, error) {
	...
	return []common.CompletionResult{}, nil
}

// convertSuggestionsToCompletions converts suggestions to completion results
func (ci *CompletionIntegration) convertSuggestionsToCompletions(suggestions []suggestions.Suggestion) []common.CompletionResult {
...
func (ci *CompletionIntegration) rankAndDeduplicate(results []common.CompletionResult, input string) []common.CompletionResult {
	if len(results) == 0 {
		return results
	}
	...
	// Extract results
	final := make([]common.CompletionResult, 0, len(scored))
...
func (ci *CompletionIntegration) calculateScore(result common.CompletionResult, input string) float64 {
	score := 0.0
