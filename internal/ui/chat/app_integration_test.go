...
func (s *mockGuildServer) ListAvailableAgents(ctx context.Context, req *guildv1.ListAgentsRequest) (*guildv1.ListAgentsResponse, error) {
	...
	agents := make([]*guildv1.Agent, 0, len(s.agents))
	for _, agent := range s.agents {
		agents = append(agents, &guildv1.Agent{
			Id:     agent.id,
			Name:   agent.name,
			Status: guildv1.AgentStatus_AGENT_STATUS_READY,
			Type:   "mock",
		})
	}
...
func (s *mockGuildServer) SendMessageToAgent(ctx context.Context, req *guildv1.AgentMessageRequest) (*guildv1.AgentMessageResponse, error) {
	...
	return &guildv1.AgentMessageResponse{
		Response: response,
		AgentId:  req.AgentId,
		Status:   guildv1.MessageStatus_MESSAGE_STATUS_SUCCESS,
	}, nil
}
	...
	for {
		...
		default:
			...
			response := &guildv1.ChatResponse{
				...
				Type:      guildv1.MessageType_MESSAGE_TYPE_TEXT,
			}
...
func TestGuildChatAgentCommunication(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Equal(t, guildv1.MessageStatus_MESSAGE_STATUS_SUCCESS, resp.Status, "Should have success status")
		})
	}
...
func TestGuildMultiAgentResponses(t *testing.T) {
	...
	t.Run("response_ordering", func(t *testing.T) {
		...
		for i, resp := range responses {
			...
			assert.Equal(t, guildv1.MessageStatus_MESSAGE_STATUS_SUCCESS, resp.Status, "All responses should be successful")
		}
	})
	...
	t.Run("status_updates", func(t *testing.T) {
		...
		for _, expectedID := range expectedAgents {
			...
			assert.Equal(t, guildv1.AgentStatus_AGENT_STATUS_READY, status, "Agent %s should be ready", expectedID)
		}
	})
...
func TestGuildChatServiceStreaming(t *testing.T) {
	...
	for i, msg := range testMessages {
		err := stream.Send(&guildv1.ChatMessage{
	...
	if len(receivedResponses) > 0 {
		for i, resp := range receivedResponses {
			assert.NotEmpty(t, resp.Id, "Response %d should have ID", i)
			assert.NotEmpty(t, resp.Message, "Response %d should have message", i)
			assert.Contains(t, resp.Message, "Echo:", "Response should be echo")
			assert.Equal(t, guildv1.MessageType_MESSAGE_TYPE_TEXT, resp.Type, "Response should be text type")
		}
	} else {
